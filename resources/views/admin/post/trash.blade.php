<x-app-layout>
    ...
    {{-- topbar --}}
    <x-topbar-content>
        <x-breadcrum href="{{ route('admin.post.list') }}" page_name="Thùng rác" />
    </x-topbar-content>
    ...
    <div class="p-3">
        <div class="additional-task bg-white shadow-md p-3 rounded-md md:flex justify-between items-center">
            ...
            <div class="hidden md:block">
                <form action="" method="POST" class="border-b w-full">
                    ...
                    <input type="text" placeholder="Nhập tiêu đề tìm kiếm..."
                        class="search_post_trash text-sm border-0 outline-none ring-0 focus:border-0 focus:outline-none focus:ring-0 pl-0" />
            ...
            <div class="flex flex-col md:flex-row gap-1 mt-2 md:mt-0 text-center">
                ...
                {{-- Quay lại --}}
                <x-a-reset href="{{ route('admin.post.list') }}" class="px-5 w-full md:w-auto">
    ...
    {{-- Bảng --}}
    <div class="data-table list_posts p-3 pt-0 {{ session('status_failed') || session('status_complete') ? '' : 'animation_content_slide_up' }}">
        @include('admin.post.partials.list_trash')
    </div>
