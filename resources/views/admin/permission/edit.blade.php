<x-app-layout>
    ...
    {{-- topbar --}}
    <x-topbar-content>
        <x-breadcrum href="{{ route('admin.permission.manager') }}" page_name="Cập nhật Quyền" />
    </x-topbar-content>
    ...
    {{ session('status_failed') || session('status_action_failed') || session('status_complete') || $errors->any() ? '' : 'animation_content_slide_up' }}">
        <div class="bg-white shadow-md rounded-md p-3 md:w-[40%]">
            <form action="{{route('admin.permission.update',$permission_info->id)}}" method="post">
                @csrf
                ...
                <div class="">
                    ...
                    <x-input type="text" name="name" id="name" value="{{ old('name',$permission_info->name) }}"/>
                ...
                <div class="mt-2">
                    ...
                    <x-input type="text" name="slug" id="slug" value="{{ old('slug',$permission_info->slug) }}"/>
                ...
                <div class="mt-2">
                    ...
                    <textarea name="desc" id="desc" cols="30" rows="10" class="w-full border-gray-300 shadow-sm rounded-md mt-2 focus:ring-green-500 focus:border-green-500">{{ old('desc',$permission_info->desc) }}</textarea>
