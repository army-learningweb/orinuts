    ...
    <form action="{{ route('admin.customers.action') }}" method="POST" id="update_all"
        ...
        <div class="md:flex gap-1 items-center">
            ...
            <select name="action" id="action"
                ...
                <option value="restore" {{ old('action') == 'restore' ? 'selected' : '' }}>Khôi phục</option>
                <option value="forceDelete" {{ old('action') == 'forceDelete' ? 'selected' : '' }}>Xóa vĩnh viễn</option>
            </select>
            {{-- button --}}
                ...
                <tbody>
                    @foreach ($customers as $customer)
                        <tr
                            ...
                            <td class="py-3 px-2 select-none w-[50px]">
                            ...
                            <td class="py-3 select-none w-[200px]">{{ $customer->tel }}</td>
                            <td class="py-3 select-none">{{ $customer->address }}</td>
                            <td class="py-3 select-none">{{ $customer->created_at }}</td>
                            <td class="py-3 text-center">
                                <a href="{{ route('admin.customers.forceDelete', $customer->id) }}"
                                    onclick="return confirm('Bạn có chắc muốn xóa khách này ra khỏi hệ thống ?')">
                                    <i
