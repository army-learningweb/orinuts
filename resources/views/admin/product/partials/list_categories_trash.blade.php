@if (count($trashs) > 0)
    {{-- Cập nhật tất cả --}}
    <form action="{{ route('admin.product.categories.action') }}" method="POST" id="update_all"
        ...
        <div class="flex justify-between items-center">
            ...
            <div class="md:flex gap-1 items-center w-full md:w-auto">
                <select name="category_action" id="action"
                    ...
                    <option value="force_delete" @if (old('category_action') == 'force_delete') selected @endif>Xóa vĩnh viễn
                    </option>
                    <option value="restore" @if (old('category_action') == 'restore') selected @endif>Khôi phục</option>
                </select>
                {{-- button --}}
                ...
                <tbody class="data-users">
                    ...
                    @foreach ($trashs as $cat)
                        ...
                        @else
                            ...
                            <td class="py-3 px-2 select-none">{{ $cat->deleted_at }}</td>
                             <td class="py-3 px-2 select-none text-gray-400">{{ $cat->deleted_at->diffForHumans()}}</td>
                            <td class="py-3 px-2 select-none">{{ $cat->user->name }}</td>
                            <td class="py-3 text-center">
                                <a href="{{ route('admin.product.categories.restore', $cat->id) }}">
                                    <i
                            ...
                            <td class="py-3 text-center">
                                <a href="{{ route('admin.product.categories.forceDelete', $cat->id) }}"
