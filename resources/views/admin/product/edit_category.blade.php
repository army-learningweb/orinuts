<x-app-layout>
    ...
    {{-- topbar --}}
    <x-topbar-content>
        <x-breadcrum href="{{route('admin.product.categories.list')}}" page_name="Cập nhật danh mục"/>
    </x-topbar-content>

    <div class="md:flex gap-3 w-full items-start p-3 @if (!$errors->any() && !session('update_complete')) animation_content_slide_up @endif">

        <form action="{{route('admin.product.categories.update',$category_details->id)}}" method="POST" class="md:w-[40%] text-sm bg-white rounded-lg shadow-md pt-1 pb-3 px-5">
            @csrf
